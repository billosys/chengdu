...
| `make warning-inventory` | compiler-warning inventory, warning ownership, and release budget | local; covered by Slice05; emits generated reports under `build/warnings/<platform>/` and leaves long-term budgets in `slice05-compiler-warning-burndown/warning-inventory.md` |
| `make sanitize-binaries` | ASan/UBSan binary sanitizer build and managed fixture runner | local; covered by Slice06; writes isolated dist under `build/binary-sanitize/<platform>/dist`; macOS LSan skipped pending Linux/toolchain re-entry |
