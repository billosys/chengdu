...
| `--supervised` positive-path fixtures for parser, grounder, and engine. | covered by Slice02 | `parser-supervised-positive`, `grounder-supervised-positive`, and `engine-supervised-positive` provide CI-safe per-component positive supervised cases; `pipeline-supervised-positive` covers the representative chain. | Re-enter for negative supervised interruption/resource cases in Slice06 if supported behavior exists. |
| TTY/color positive cases for `--color=auto`, `--color=never`, and human-stderr-only ANSI policy. | Arc06 Slice06 | Current color fixtures focus on machine-stream ANSI absence and illegal color ownership; Slice02 adds supervised ANSI-free positives but not deterministic TTY simulation. | Add only if the harness can make TTY/no-TTY behavior deterministic. |
| Unreadable-input fixtures where the platform can create them safely. | Arc06 Slice06 | Missing-input coverage exists; permission-based unreadability can be platform-sensitive on local/CI hosts. | Add only with a deterministic platform predicate or skip classification. |
...
| Binary-owned timeout and resource-limit status fixtures. | Arc06 Slice06 | Arc03 names the status classes, but the current binaries do not expose accepted timeout/resource options. | Re-enter only if a supported timeout/resource option or harness-observable contract is accepted. |
