# CC assignment - chengdu arc01 / slice03 - engine-audit
...
7. `/Users/oubiwann/lab/billosys/ai-engineering/knowledge/cpp/SKILL.md` -
   ...
   `guides/14-c-style-and-modernization.md` for this engine audit.
8. `docs/design-v0.3.0/arc01-vendored-source-audit/slice03-engine-audit/slice-doc.md`
   - goal, scope, constraints, exit criteria.
9. `docs/design-v0.3.0/arc01-vendored-source-audit/slice03-engine-audit/ledger.md`
...
Produce a complete engine audit report at:

`docs/design-v0.3.0/arc01-vendored-source-audit/audit-results-pandapi-engine.md`

Audit `pandaPI/pandaPIengine` as inherited C/C++ source that must become a
...
- error handling and status/exit behavior, including solved/unsolvable,
  timeout/resource, parse/input, and internal-error classification;
- stdout/stderr, buffering/flushing, ANSI/color, and TTY/pipe behavior,
  especially separation of plan output from diagnostics/progress;
- signal/resource behavior and search lifecycle ownership visible from source;
- CLI/API surface, including `options.ggo`, `problemSolver.sh`,
  help/version behavior, and implications for `pandapi-*` binary names;
- build/test coverage, CMake/generated-code assumptions, and fixture posture;
...
- Do not change engine source, scripts, workflows, release assets, or build
  outputs. This is diagnosis only.
- Do not produce generic advice. Every finding cites an exact
  `pandaPI/pandaPIengine/...:<line>` location.
...
All 14 ledger rows reach final status; the engine audit report exists at the
