# CC assignment - chengdu arc01 / slice02 - grounder-audit
...
6. `docs/design-v0.3.0/arc01-vendored-source-audit/audit-results-pandapi-parser.md`
   - accepted parser audit baseline and synthesis inputs to compare against.
7. `/Users/oubiwann/lab/billosys/ai-engineering/knowledge/cpp/SKILL.md` -
   ...
   `guides/08-concurrency.md`, `guides/09-performance.md`,
   `guides/10-expressions-and-statements.md`,
   `guides/12-project-structure-and-tooling.md`,
   `guides/13-standard-library.md`, and
   `guides/14-c-style-and-modernization.md` for this grounder audit.
8. `docs/design-v0.3.0/arc01-vendored-source-audit/slice02-grounder-audit/slice-doc.md`
   - goal, scope, constraints, exit criteria.
9. `docs/design-v0.3.0/arc01-vendored-source-audit/slice02-grounder-audit/ledger.md`
...
Produce a complete grounder audit report at:

`docs/design-v0.3.0/arc01-vendored-source-audit/audit-results-pandapi-grounder.md`

Audit `pandaPI/pandaPIgrounder` as inherited C/C++ source that must become a
maintainable component in a high-quality managed-process toolchain. Cover:

- source/build/test inventory;
- grounder-owned integration with `cpddl` and `h2-fd-preprocessor`, explicitly
  separated from dependency-internal audit scope;
...
- build/test coverage, generated-code assumptions, dependency build coupling,
  and fixture posture;
...
1. Executive summary.
2. Audit context: date, branch, project root, audited paths, commands run,
   dependency-boundary statement, and the C++ knowledge-pack guides loaded.
3. Findings grouped by category. Each finding has Severity, Location, What's
   ...
   Fix / recommendation.
4. Dependency boundary notes: grounder-owned integration vs dependency-internal
   findings, with recommended re-entry conditions for deeper dependency audits.
5. Cross-codebase synthesis inputs.
6. Things I looked for and did not find - at least five clean checks.

## Constraints

- Do not change grounder source, dependency source, scripts, workflows, release
  assets, or build outputs. This is diagnosis only.
- Do not produce generic advice. Every finding cites an exact
  `pandaPI/pandaPIgrounder/...:<line>` location, or clearly labels a dependency
  location as dependency-internal and not part of this slice's repair scope.
- Map source-quality findings to C++ Core Guidelines rule IDs where applicable.
  Keep chengdu-specific managed-process findings, such as stdout/stderr or pipe
  behavior, clearly labeled when no Core Guidelines rule directly applies.
...
- If you run builds/tests/probes, record exact commands and exit status. If
  sandbox, platform assumptions, or missing dependencies block a useful command,
  record the blocker.
...
All 14 ledger rows reach final status; the grounder audit report exists at the
planned arc-local path; findings are concrete, file/line-cited, dependency
boundaries are explicit, and source-quality findings are mapped to C++ Core
Guidelines rule IDs where applicable; at least five clean checks are recorded;
synthesis inputs are explicit; and no planner source/build/release files
changed.
