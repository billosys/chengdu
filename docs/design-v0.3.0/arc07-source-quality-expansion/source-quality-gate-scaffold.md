...
Status: Slice07 implementation report
...
| format | grounder | first-party maintained and dependency exclusions | `format-check-grounder`; aggregate `format-check-first-party` still available | Executable baseline records current drift without changing inherited formatting. | `build/source-quality/<platform>/format-check-grounder.md` | Convert recorded drift to strict formatting only after a grounder formatting burndown is accepted. |
...
| static analysis | grounder | first-party maintained plus dependency exclusions | `static-analysis-grounder`; aggregate `static-analysis-first-party` still available | Executable clang-tidy and clang-analyzer baseline over `compile-db-grounder` selected units; findings are reported, not blanketed. | `build/source-quality/<platform>/static-analysis-grounder.md` | Decide which analyzer findings become strict after the grounder baseline is accepted. |
...
| coverage | grounder | first-party maintained plus dependency exclusions | `coverage-grounder`; aggregate `coverage-first-party` still available | Executable topsort seam coverage baseline with generated, cpddl, H2, nested third-party, copied-build, and BUILD_OUTPUT routing. | `build/source-quality/<platform>/coverage-grounder.md` | Expand only through maintained-source seams or mapped profiles; no release floor. |
...
| unit | grounder | first-party maintained | `test-unit-grounder`; aggregate `test-unit` still available | Executable topsort seam evidence for maintained grounder logic. | `build/source-quality/<platform>/test-unit-grounder.md` | Add more grounder CTest or Catch2 seams after risk-bearing internals are selected. |
...
| warning | grounder | first-party warning, generated, dependency, third-party classes | `warning-inventory-grounder`; aggregate `warning-inventory-first-party` still available | Executable inventory separates first-party warnings from generated cmdline, cpddl, H2, nested third-party, copied, and build-output classes. | `build/source-quality/<platform>/warning-inventory-grounder.md` | Dependency audit or support decision required before dependency warning budget becomes release blocker. |
...
| sanitizer | grounder | first-party maintained, dependency, third-party, copied-build | `sanitize-grounder`; `sanitize-first-party` and existing binary gate `test-binary-sanitize` still available | Executable ASan/UBSan topsort seam; binary sanitizer remains existing local/pre-release gate with dependency triage. | `build/source-quality/<platform>/sanitize-grounder.md` | Re-enter on Linux LSan, TSan, binary sanitizer, or dependency findings; classify cpddl/H2 separately. |
...
## Slice07 Grounder Source-Quality Boundary Status

- `format-check-grounder`, `static-analysis-grounder`, `coverage-grounder`, `warning-inventory-grounder`, `test-unit-grounder`, and `sanitize-grounder` are named Make targets with durable reports.
- `compile-db-grounder` selects grounder first-party maintained translation units and excludes generated `cmdline.c`/`cmdline.h`, cpddl, H2, nested third-party source, copied build roots, and BUILD_OUTPUT roots from first-party gate interpretation.
- Generated `cmdline.c`/`cmdline.h` remain generated build output from `options.ggo`; private generated headers under `build/source-quality/<platform>/compile-db/grounder/generated/` and `build/source-quality/<platform>/compile-db/grounder/cpddl-generated/` support static-analysis compilation without committing generated output.
- `rss.c` remains first-party maintained for grounder gate purposes because it is shipped and selected from `pandaPI/grounder/src`; its copied-origin RSS utility status is recorded separately, and its strict-prototype warnings are fixed in source.
- Grounder dependency warnings from cpddl, boruvka, opts, lpsolve, and H2 remain dependency or third-party warning budgets with dependency-audit re-entry.
- `test-unit-grounder`, `coverage-grounder`, and `sanitize-grounder` exercise the maintained `topsort` seam directly; managed contract fixtures remain process-level contract evidence.
