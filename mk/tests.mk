...
.PHONY: test-contract-engine-managed
test-contract-engine-managed: build-engine
	printf '%b\n' "$(BLUE)Running managed engine contract fixtures...$(RESET)"
	./tests/contract/run --contract --component engine
	printf '%b\n' "$(GREEN)Managed engine contract fixtures passed$(RESET)"
...
.PHONY: test-contract-stdio-managed
test-contract-stdio-managed: build
	printf '%b\n' "$(BLUE)Running managed stdio contract fixtures...$(RESET)"
	./tests/contract/run --contract --component stdio
	printf '%b\n' "$(GREEN)Managed stdio contract fixtures passed$(RESET)"
...
.PHONY: test
test: build test-runtime test-contract test-contract-parser-managed test-contract-grounder-managed test-contract-engine-managed test-contract-pipeline-managed test-contract-stdio-managed smoke smoke-negative
