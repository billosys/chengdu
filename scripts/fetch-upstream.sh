...
Usage: fetch-upstream.sh [--source canonical|pandadealer]
  ...
  --source pandadealer  clone the ipc2023-htn/PandaDealer vendored
                         ...
                         If your network can't reach gitlab.com (the
                         cpddl submodule's host), prefer installing a
                         v0.1.0+ release instead — it needs no gitlab.com
                         reachability at all. This mode is a historical/
                         source-inspection fallback only: FETCH-ONLY, the
                         vendored snapshot ships no patch files and its
                         grounder/engine sources don't build cleanly on a
                         modern Linux/GCC toolchain (found 2026-08-05:
                         pre-final concepts-TS syntax GCC-13 rejects, and
                         a missing <cstdint> include). build-grounder.sh
                         refuses to run against it. Canonical mode is the
                         only buildable source from this repo today.
EOF
}
