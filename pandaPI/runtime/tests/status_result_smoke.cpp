...
bool check_status_case(const StatusCase& test_case)
{
  const auto status = pandapi::runtime::ProcessStatus::from_code(
      test_case.code, pandapi::runtime::Component::Engine,
...
int main()
{
  ...
  const std::array<StatusCase, 16> cases{{
      ...
      {StatusCode::DomainNoPlan, "domain_no_plan", 2,
       StatusClass::ExpectedDomainOutcome},
      ...
      {StatusCode::OutputUnavailable, "output_unavailable", 21,
       StatusClass::CallerError},
      {StatusCode::InputInvalid, "input_invalid", 22, StatusClass::InputModelError},
      {StatusCode::UnsupportedFeature, "unsupported_feature", 30,
       StatusClass::PolicySurfaceFailure},
      {StatusCode::LegacySurface, "legacy_surface", 31,
       StatusClass::PolicySurfaceFailure},
      {StatusCode::ExperimentalSurface, "experimental_surface", 32,
       StatusClass::PolicySurfaceFailure},
      {StatusCode::FutureSurface, "future_surface", 33,
       StatusClass::PolicySurfaceFailure},
      {StatusCode::Timeout, "timeout", 40,
       StatusClass::TimeoutResourceInterruptionFailure},
      {StatusCode::ResourceLimit, "resource_limit", 41,
       StatusClass::TimeoutResourceInterruptionFailure},
      {StatusCode::Interrupted, "interrupted", 42,
       StatusClass::TimeoutResourceInterruptionFailure},
      {StatusCode::DependencyFailure, "dependency_failure", 50,
       StatusClass::DependencyChildProcessFailure},
      {StatusCode::ChildProcessFailure, "child_process_failure", 51,
       StatusClass::DependencyChildProcessFailure},
  ...
  const auto signal_status = pandapi::runtime::ProcessStatus::signal_terminated(
      15, pandapi::runtime::Component::Engine,
      pandapi::runtime::SurfaceDisposition::Supported);
  ...
  if (pandapi::runtime::exit_code(signal_status) !=
      128 + signal_status.signal_number()) {
    return 4;
  }
  ...
  if (pandapi::runtime::component_name(pandapi::runtime::Component::Parser) !=
          "parser" ||
      pandapi::runtime::component_name(pandapi::runtime::Component::Grounder) !=
          "grounder" ||
      pandapi::runtime::component_name(pandapi::runtime::Component::Engine) !=
          "engine") {
    return 7;
  }

  if (pandapi::runtime::surface_disposition_name(
          pandapi::runtime::SurfaceDisposition::Experimental) != "experimental") {
    return 8;
  }
