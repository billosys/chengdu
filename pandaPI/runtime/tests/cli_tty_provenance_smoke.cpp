...
{
  const auto parser =
      pandapi::runtime::command_identity_for(Component::Parser, "pandapi-parser");
  assert(parser.has_value());
  assert(parser.value().canonical_name() == "pandapi-parser");
  assert(!parser.value().is_compatibility_invocation());

  const auto grounder =
      pandapi::runtime::command_identity_for(Component::Grounder, "pandaPIgrounder");
  ...
  const auto mismatch =
      pandapi::runtime::command_identity_for(Component::Engine, "pandaPIparser");
...
void cli_policy_conflicts_are_cli_usage_error()
{
  ...
  const auto data_result = pandapi::runtime::validate_common_cli_options(
      data_stdout_conflict, Component::Parser);
  ...
  const auto informational_result = pandapi::runtime::validate_common_cli_options(
      informational_conflict, Component::Grounder);
  ...
  const auto alias_result =
      pandapi::runtime::validate_common_cli_options(alias_conflict, Component::Parser);
  ...
  const auto color_result =
      pandapi::runtime::validate_common_cli_options(color_conflict, Component::Engine);
  ...
  const auto status_stderr_color_result = pandapi::runtime::validate_common_cli_options(
      status_stderr_color_conflict, Component::Engine);
  ...
  const auto status_stdout_color_result = pandapi::runtime::validate_common_cli_options(
      status_stdout_color_conflict, Component::Engine);
  ...
  const auto disabled_color_result =
      pandapi::runtime::validate_common_cli_options(disabled_color, Component::Engine);
  ...
  const auto disabled_status_color_result =
      pandapi::runtime::validate_common_cli_options(disabled_status_color,
                                                    Component::Engine);
...
void informational_commands_are_stdout_owned_successes()
{
  const auto command = pandapi::runtime::informational_command_from_flags(
      false, true, false, Component::Engine);
  ...
  const auto status = pandapi::runtime::informational_command_status(command.value(),
                                                                     Component::Engine);
  ...
  const auto multiple = pandapi::runtime::informational_command_from_flags(
      true, true, false, Component::Engine);
...
void color_policy_is_injected_and_keeps_machine_streams_clean()
{
  ...
  const AnsiPolicy human_stderr{
      enabling, TerminalKind::Tty, AnsiStream::Stderr, OutputRole::Empty, false, false};
  ...
  const AnsiPolicy stdout_artifact{
      enabling, TerminalKind::Tty, AnsiStream::Stdout, OutputRole::DataArtifact, false,
      false};
  assert(!pandapi::runtime::allows_color(stdout_artifact));

  const AnsiPolicy tagged_status{
      enabling, TerminalKind::Tty, AnsiStream::Stderr, OutputRole::TaggedStatus, false,
      false};
  assert(!pandapi::runtime::allows_color(tagged_status));

  const AnsiPolicy supervised_pipe{
      enabling, TerminalKind::NotTty, AnsiStream::Pipe, OutputRole::Empty, true, true};
  assert(!pandapi::runtime::allows_color(supervised_pipe));

  const AnsiPolicy file_output{
      enabling, TerminalKind::Tty, AnsiStream::File, OutputRole::Empty, false, false};
...
void provenance_output_is_stable_and_caller_supplied()
{
  ...
  const auto version_without_invoked =
      pandapi::runtime::format_version(canonical_invocation, Component::Engine);
  assert(version_without_invoked.has_value());
  assert(version_without_invoked.value().find("invoked_command=") == std::string::npos);

  ProvenanceRecord missing_required;
  missing_required.canonical_command = "pandapi-engine";
  const auto missing_required_result =
      pandapi::runtime::format_version(missing_required, Component::Engine);
  ...
  const auto provenance =
      pandapi::runtime::format_provenance(record, Component::Engine);
  ...
  const auto omitted =
      pandapi::runtime::format_provenance(missing_optional, Component::Engine);
  ...
  const auto rejected =
      pandapi::runtime::format_version(placeholder, Component::Engine);
...
{
  const auto color = pandapi::runtime::parse_color_mode("rainbow", Component::Engine);
  assert(!color.has_value());
  assert(color.status().code() == StatusCode::CliUsageError);

  const auto status_target =
      pandapi::runtime::parse_status_target("both", Component::Engine);
