...
public:
  [[nodiscard]] int flush_count() const noexcept { return flush_count_; }
...
[[nodiscard]] pandapi::runtime::ProcessStatus status(pandapi::runtime::StatusCode code)
{
  return pandapi::runtime::ProcessStatus::from_code(
      code, pandapi::runtime::Component::Engine,
...
[[nodiscard]] bool check_round_trip_success()
{
  auto record = pandapi::runtime::StatusRecord::create(
      status(pandapi::runtime::StatusCode::Ok), "normal_search");
  if (!record.has_value()) {
    return false;
  }
...
{
  auto record = pandapi::runtime::StatusRecord::create(
      status(pandapi::runtime::StatusCode::DomainNoPlan), "normal_search");
  if (!record.has_value()) {
    return false;
  }
...
{
  return pandapi::runtime::status_stream_name(pandapi::runtime::StatusStream::Stderr) ==
             "stderr" &&
         pandapi::runtime::status_stream_name(pandapi::runtime::StatusStream::Stdout) ==
             "stdout" &&
         pandapi::runtime::status_stream_allowed(
             pandapi::runtime::StatusStream::Stderr,
             pandapi::runtime::OutputRole::DataArtifact) &&
         pandapi::runtime::status_stream_allowed(pandapi::runtime::StatusStream::Stdout,
...
[[nodiscard]] bool check_single_line_escape_and_reject()
{
  ...
  return newline == std::string::npos && escaped_newline != std::string::npos &&
...
{
  auto missing_tag =
      pandapi::runtime::parse_status_record("status=ok\tcomponent=engine");
  ...
  if (non_digit_exit.has_value() ||
      non_digit_exit.status().code() != pandapi::runtime::StatusCode::InputInvalid) {
    return false;
  }
  ...
  if (oversized_exit.has_value() ||
      oversized_exit.status().code() != pandapi::runtime::StatusCode::InputInvalid) {
    return false;
  }
  ...
  return !oversized_signal.has_value() &&
         oversized_signal.status().code() == pandapi::runtime::StatusCode::InputInvalid;
}
...
[[nodiscard]] bool check_flushing()
{
  ...
  return buffer.flush_count() == 1 &&
         buffer.str().find("PANDAPI_STATUS\tstatus=ok") == 0 && !buffer.str().empty() &&
         buffer.str().back() == '\n';
}
