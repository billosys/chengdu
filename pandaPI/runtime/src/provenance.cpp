...
[[nodiscard]] StatusResult<bool>
add_required_field(std::vector<ProvenanceField>& fields, FieldView field,
                   Component component, SurfaceDisposition surface_disposition)
{
  ...
  fields.push_back(ProvenanceField{std::string{field.name}, std::string{field.value}});
  return StatusResult<bool>::success(true);
}

[[nodiscard]] StatusResult<bool>
add_field_if_present(std::vector<ProvenanceField>& fields, std::string name,
...
render_fields(const std::vector<ProvenanceField>& fields, Component component,
              SurfaceDisposition surface_disposition)
{
  if (fields.empty()) {
...
std::string_view provenance_record_kind() noexcept { return "stable field"; }

StatusResult<std::vector<ProvenanceField>>
               ...
               SurfaceDisposition surface_disposition)
{
  ...
  for (const auto field : required_fields) {
    const auto add_required =
        add_required_field(fields, field, component, surface_disposition);
    if (!add_required.has_value()) {
      return StatusResult<std::vector<ProvenanceField>>::failure(add_required.status());
    }
  }

  const auto add_invoked_command =
      add_field_if_present(fields, "invoked_command", record.invoked_command, component,
                           surface_disposition);
...
StatusResult<std::vector<ProvenanceField>>
provenance_fields(const ProvenanceRecord& record, Component component,
                  SurfaceDisposition surface_disposition)
{
  ...
  const auto add_import_commit = add_field_if_present(
      fields, "import_commit", record.import_commit, component, surface_disposition);
  if (!add_import_commit.has_value()) {
    return StatusResult<std::vector<ProvenanceField>>::failure(
        add_import_commit.status());
  }

  const auto add_build_timestamp =
      add_field_if_present(fields, "build_timestamp", record.build_timestamp, component,
                           surface_disposition);
  ...
  const auto add_third_party_licenses =
      add_field_if_present(fields, "third_party_licenses", record.third_party_licenses,
                           component, surface_disposition);
...
StatusResult<std::string> format_version(const ProvenanceRecord& record,
                                         Component component,
                                         SurfaceDisposition surface_disposition)
{
...
StatusResult<std::string> format_provenance(const ProvenanceRecord& record,
                                            Component component,
                                            SurfaceDisposition surface_disposition)
{
  const auto fields_result = provenance_fields(record, component, surface_disposition);
  if (!fields_result.has_value()) {
    return StatusResult<std::string>::failure(fields_result.status());
  }
