...
normalize_paths(std::string_view text,
                const std::vector<PathReplacement>& replacements);
[[nodiscard]] StatusResult<std::string> normalize_text(std::string_view text,
                                                       const NormalizationRules& rules);
[[nodiscard]] bool duration_is_non_negative_ms(std::string_view duration) noexcept;
[[nodiscard]] bool duration_within_bound(std::chrono::milliseconds duration,
...
[[nodiscard]] bool errno_matches_symbolic(std::string_view observed, int errno_value);
...
[[nodiscard]] ComparisonResult comparison_mismatch(std::string field,
                                                   std::string expected,
                                                   std::string actual,
                                                   std::string mismatch);
[[nodiscard]] ComparisonResult compare_golden(std::string field,
                                              std::string_view expected,
                                              std::string_view actual,
                                              const NormalizationRules& rules);
[[nodiscard]] ComparisonResult compare_semantic_predicate(std::string field,
                                                          bool predicate_matches,
                                                          std::string expected,
                                                          std::string actual);
[[nodiscard]] StatusResult<StatusRecord> parse_final_status(std::string_view line);
[[nodiscard]] ComparisonResult match_status_record(const StatusRecord& record,
