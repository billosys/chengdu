...
{
  FixtureSpec fixture;
  fixture.id = "fixture record normalization status matching comparison process observation timeout";
  fixture.owner = "Arc04";
  ...
  fixture.command.environment.variables.push_back(EnvironmentVariable{
    "PANDAPI_TEST_ROOT",
    "/tmp/pandapi-fixture"});
  fixture.stdout_expectation = StreamExpectation{
    StreamRole::DataArtifact,
    ComparisonPolicy::Golden,
    "fixture-stdout",
    false};
  fixture.stderr_expectation = StreamExpectation{
    StreamRole::HumanDiagnostics,
    ComparisonPolicy::Semantic,
    "fixture-stderr",
    true};
  fixture.output_artifacts.push_back(OutputArtifactExpectation{
    "$TMPDIR/plan.txt",
    "plan",
  ...
  fixture.final_status.required_fields = {
    ...
    "exit_code",
    "class"};
  fixture.normalization.path_tokens.push_back(PathReplacement{
    PathToken::TempRoot,
    "/tmp"});
  fixture.safety.cleanup = CleanupPolicy::RemoveTempArtifacts;
  fixture.safety.probes.push_back(ProbePolicy{
    ProbeKind::HarnessTimeout,
    ProbeSafety::CiSafe,
    std::chrono::milliseconds{50}});
  fixture.safety.probes.push_back(ProbePolicy{
    ProbeKind::BinaryTimeout,
    ProbeSafety::Quarantine,
    std::chrono::milliseconds{0}});
  fixture.safety.probes.push_back(ProbePolicy{
    ProbeKind::ResourceLimit,
    ProbeSafety::Manual,
    std::chrono::milliseconds{0}});
  return fixture;
}
...
void fixture_records_are_typed()
{
  ...
  assert(pandapi::runtime::probe_kind_name(ProbeKind::HarnessTimeout)
    == "HarnessTimeout");
  assert(pandapi::runtime::probe_safety_name(ProbeSafety::Quarantine)
    == "quarantine");
  assert(pandapi::runtime::probe_safety_name(ProbeSafety::Manual)
    == "manual");
  assert(pandapi::runtime::probe_safety_name(ProbeSafety::CiSafe)
    == "CI-safe");
...
void normalization_and_comparison_helpers_work()
{
  ...
  const auto normalized = pandapi::runtime::normalize_text(
    "\x1b[31m/fixtures/domain.hddl\r\n",
    rules);
  ...
  assert(pandapi::runtime::duration_within_bound(
    std::chrono::milliseconds{37},
    std::chrono::milliseconds{50}));
  assert(pandapi::runtime::timestamp_matches_rfc3339_like(
  ...
  const auto golden = pandapi::runtime::compare_golden(
    "stdout",
    "$FixtureRoot/domain.hddl\n",
    "/fixtures/domain.hddl\r\n",
    rules);
  assert(golden.matches);

  const auto mismatch = pandapi::runtime::compare_golden(
    "stdout",
    "expected",
    "actual",
    NormalizationRules{});
...
void final_status_matching_uses_status_parser()
{
  ...
  const auto matched = pandapi::runtime::match_status_record(
    parsed.value(),
    StatusCode::Ok);
  assert(matched.matches);

  const auto mismatch = pandapi::runtime::match_status_record(
    parsed.value(),
...
void process_observation_captures_success_and_failure()
{
  ...
  const auto observed = pandapi::runtime::run_process_fixture(
    command,
    RunOptions{std::chrono::milliseconds{500}});
  ...
  assert(observed.value().wait_status.kind
    == pandapi::runtime::WaitStatusKind::Exited);
...
void harness_timeout_is_ci_safe_and_bounded()
{
  ...
  const auto observed = pandapi::runtime::run_process_fixture(
    command,
    RunOptions{std::chrono::milliseconds{50}});
  assert(observed.has_value());
  assert(observed.value().timed_out);
  ...
  assert(observed.value().cleanup_state
    == pandapi::runtime::CleanupState::Completed);
  assert(observed.value().wait_status.kind
